...
class MLPL(torch.nn.Module):
    def __init__(self, in_size, out_size, layers, act="ReLU"):
        super().__init__()
...
class StableResNODEMLP(nn.Module):
    def __init__(self, in_size, out_size, mlp_size, num_layers, act="SiLU"):
        super().__init__()
...
class Hypernetwork(TorchHyperNetwork):
    def __init__(
        self,
    ...
    def make_weight_generator(self) -> nn.Module:
        ...
        if len(self.layers)==0:  
            ...
            if self.skip:
                return StableResNODEMLP(self.hyper_dims, self.num_target_parameters,self.layers[0], len(self.layers),self.act)
            else:
                return MLPL(self.hyper_dims, self.num_target_parameters, self.layers, self.act)
